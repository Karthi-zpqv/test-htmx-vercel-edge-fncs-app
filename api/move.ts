...
function isBasicMoveLegal(from: string, to: string, boardState: any, playerColor: string): boolean {
  ...
  if ((playerColor === 'white' && !piece.startsWith('w')) || (playerColor === 'black' && !piece.startsWith('b'))) {
    return false;
  }
...
export default async function handler(req: NextRequest) {
  ...
  try {
    ...
    const result = await db.runTransaction(async (trx) => {
      ...
      if (playerIdx === -1 || !Array.isArray(data?.players)) throw new Error('forbidden');

      // Check not game over
      if (data?.status === 'over' || data?.result?.status === 'over') throw new Error('game_over');
      ...
      const moves = Array.isArray(data.moves) ? [...data.moves, newMove] : [newMove];
